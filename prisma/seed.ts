...
async function main() {
  console.log('Seeding initial admin and demo data...');
  ...
  // Generate 80 daily schedule items for Ravi
  const raviSchedules = [];
  for (let i = 0; i < 80; i++) {
    const sDate = new Date(startDate);
    sDate.setDate(startDate.getDate() + i);
    const isPast = i < 10; // First 10 days passed
    raviSchedules.push({
  ...
  for (let i = 0; i < 70; i++) {
    const sDate = new Date(startDate);
    sDate.setDate(startDate.getDate() + i);
    const isPastPaid = i < 8; // Paid 8 days
    const isMissed = i === 8 || i === 9; // Missed days 9 and 10
